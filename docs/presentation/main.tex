...
\begin{frame}{Estimating parameters}
    Once the model is chosen, it is necessary estimate its parameters vector \(a\)
    \bigskip 
...
\begin{frame}{Indirect parameter estimation}
    ...
    \begin{block}{Minimizing the error}
        Assuming \(p = 2\) we can recompute \(a\) minimizing \(E_{\textrm{fit}}\) as follows
...
\begin{frame}{Overview}
    ...
    \bigskip
    \begin{enumerate}
        \item \bold{motion estimation}: compute a dense motion field for a given frame to use as ground truth
        \item \bold{global motion estimation}: estimate the best parameters for the affine model
